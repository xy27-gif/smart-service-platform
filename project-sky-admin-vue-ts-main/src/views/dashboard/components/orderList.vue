              ...
              <template slot-scope="scope">
                <div class="ellipsisHidden">
                  <el-popover
                    ...
                    width="200"
                    trigger="hover"
                    :content="getServiceItems(scope.row)"
                  >
                    <span slot="reference">{{ getServiceItems(scope.row) }}</span>
                  </el-popover>
                </div>
              </template>
            </el-table-column>
...
export default class extends Vue {
  ...
  // 获取服务项目列表
  private getServiceItems(row) {
    // 优先使用 orderServiceItems（后端返回的字段）
    if (row.orderServiceItems) {
      return row.orderServiceItems.replace(/;/g, '、').replace(/\*1/g, '').replace(/、$/, '')
    }
    // 优先使用 orderDetailList
    if (row.orderDetailList && row.orderDetailList.length > 0) {
      return row.orderDetailList.map(item => `${item.name} x${item.number}`).join('、')
    }
    // 回退使用 orderDishes
    if (row.orderDishes) {
      return row.orderDishes
    }
    return '无服务项目'
  }
  get tabList() {
  ...
  async getOrderListData(status) {
    ...
    console.log('订单数据:', data.data.data.records)
